...
void  TFT_eSPI::readRectRGB(int32_t x0, int32_t y0, int32_t w, int32_t h, uint8_t *data)
{
  ...
  while (len--) {
    ...
    *data++ = blue;
    ...
    Serial.println("[TFT] begin_tft_read()");
  begin_tft_read();
    Serial.println("[TFT] begin_tft_read() ok");

    Serial.println("[TFT] readAddrWindow()");
  readAddrWindow(x0, y0, w, h); // Sets CS low
    Serial.println("[TFT] readAddrWindow() ok");

  #ifdef TFT_SDA_READ
    begin_SDA_Read();
  #endif

    Serial.println("[TFT] tft_Read_8()");
  // Dummy read to throw away don't care value
  uint8_t gg = tft_Read_8();
  Serial.printf("[TFT] tft_Read_8() is %d\n", gg);
    Serial.println("[TFT] tft_Read_8() ok");

  // Read window pixel 24 bit RGB values, buffer must be set in sketch to 3 * w * h
  uint32_t len = w * h;
  while (len--) {
      Serial.print("[TFT] len: ");
      Serial.println(len);
...
//   Serial.printf("[TFT] tft_Read_8() is %d\n", gg);
  ...
  CS_H;
    ...
    Serial.println("[TFT] end_tft_read()");
  end_tft_read();
    Serial.println("[TFT] end_tft_read() ok");
